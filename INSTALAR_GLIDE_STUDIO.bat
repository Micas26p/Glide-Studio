...
REM -----------------------------------------------------------
REM ETAPA 5: COMPILACAO DO EXECUTAVEL STANDALONE (Glide Studio.exe)
REM -----------------------------------------------------------
echo [5/6] Verificando executavel standalone Glide Studio.exe...
if not exist "%~dp0Glide Studio.exe" (
    echo Compilando Glide Studio.exe standalone para uso direto sem terminal...
    pyinstaller --clean --noconfirm --distpath "%~dp0." "%~dp0GlideUltra.spec"
    if exist "%~dp0build" rmdir /s /q "%~dp0build" 2>nul
    if exist "%~dp0Glide Studio.exe" (
        echo [OK] Glide Studio.exe compilado com sucesso!
    ) else (
        echo [AVISO] Nao foi possivel compilar o EXE standalone. O sistema funcionara normalmente via iniciar.bat.
    )
) else (
    echo [OK] Glide Studio.exe ja se encontra pronto para uso direto.
)
echo.

REM -----------------------------------------------------------
REM ETAPA 6: CRIACAO DE ATALHOS NA AREA DE TRABALHO
REM -----------------------------------------------------------
echo [6/6] Criando atalho na Area de Trabalho...
powershell -NoProfile -Command "$wsh = New-Object -ComObject WScript.Shell; $desktop = [Environment]::GetFolderPath('Desktop'); $s = $wsh.CreateShortcut(\"$desktop\Glide Studio.lnk\"); if (Test-Path '%~dp0Glide Studio.exe') { $s.TargetPath = '%~dp0Glide Studio.exe' } else { $s.TargetPath = '%~dp0iniciar.bat' }; $s.WorkingDirectory = '%~dp0'; if (Test-Path '%~dp0assets\icon.ico') { $s.IconLocation = '%~dp0assets\icon.ico' }; $s.Save()" 2>nul
...
echo ============================================================
echo.
echo   O Glide Studio esta totalmente configurado para uso continuo!
echo   - Nao e necessario rodar este instalador novamente.
echo.
echo   COMO USAR NO DIA A DIA:
echo   1. MODO DESKTOP (Janela Nativa):
echo      - Abra diretamente pelo executavel: Glide Studio.exe
echo      - Ou pelo atalho criado na sua Area de Trabalho
echo      - Ou clicando em iniciar.bat
echo.
echo   2. MODO WEB (No Navegador):
echo      - Basta dar 2 cliques em: Iniciar_Versao_Web.bat
echo ============================================================
echo.
...
if /i "!RESP!"=="N" (
    echo.
    echo Perfeito! Voce pode abrir a qualquer momento pelo atalho ou executavel.
    echo Ate logo!
